...
from vigil_overlay.services.fps_sampling import (
    presentmon_permission_required as _presentmon_permission_required,
)
from vigil_overlay.services.fps_sampling import (
    unavailable_fps_metric as _unavailable_fps_metric,
)
...
class PresentMonFpsService(QObject):
    ...
    # These compatibility accessors keep existing focused tests and diagnostic
    # tooling usable while all state is physically owned by one boundary object.
    @property
    def _target(self) -> FpsTarget | None:
        return self._ownership.target

    @_target.setter
    def _target(self, target: FpsTarget | None) -> None:
        self._ownership.target = target

    @property
    def _last_capture_diagnostics(self) -> PresentMonCaptureDiagnostics | None:
        return self._ownership.last_capture_diagnostics

    @_last_capture_diagnostics.setter
    def _last_capture_diagnostics(
        self,
        diagnostics: PresentMonCaptureDiagnostics | None,
    ) -> None:
        self._ownership.last_capture_diagnostics = diagnostics

    @property
    def _stream_identity(self) -> tuple[int, int | str] | None:
        return self._ownership.stream_identity

    @_stream_identity.setter
    def _stream_identity(self, identity: tuple[int, int | str] | None) -> None:
        self._ownership.stream_identity = identity

    @property
    def _verified_target_identity(self) -> tuple[int, int | str] | None:
        return self._ownership.verified_target_identity

    @_verified_target_identity.setter
    def _verified_target_identity(self, identity: tuple[int, int | str] | None) -> None:
        self._ownership.verified_target_identity = identity

    @property
    def _stream_selector(self) -> FpsStreamSelector:
        return self._ownership.stream_selector

    @_stream_selector.setter
    def _stream_selector(self, selector: FpsStreamSelector) -> None:
        self._ownership.stream_selector = selector
    ...
    def _replace_capture(
        ...
        *,
        ...
        join_worker(
            previous_thread,
            timeout_seconds=2.0,
            worker_name="FPS capture supervisor",
            logger=_LOGGER,
        )
    ...
    def _selector_for_target(self, target: FpsTarget) -> FpsStreamSelector:
        with self._lock:
            return self._ownership.selector_for(target)
    ...
    def _clear_finished_target(self, generation: int, target: FpsTarget) -> None:
        ...
        with self._lock:
            if generation != self._generation:
                return
            if not self._ownership.clear_if_current(target):
                return
            self._preferred_target = None
            self._pending_foreground_hint = None
            self._discovery_active.set()
    ...
    def _consume_process(
        ...
        join_worker(
            reader,
            timeout_seconds=1.0,
            worker_name="PresentMon output reader",
            logger=_LOGGER,
        )
        if stderr_reader is not None:
            join_worker(
                stderr_reader,
                timeout_seconds=1.0,
                worker_name="PresentMon diagnostic reader",
                logger=_LOGGER,
            )
        ...
        if (
            ...
            outcome = _CaptureOutcome.PERMISSION_REQUIRED
        with self._lock:
            self._ownership.record_diagnostics_if_current(target, diagnostics)
        return outcome
    ...
    def _retains_verified_capture_locked(self, target: FpsTarget) -> bool:
        return self._ownership.retains_verified(target)
