"""Bounded filesystem adapter used by the native Epic Games provider."""
...
class EpicFileSystem(Protocol):
    """Bounded filesystem operations required by Epic discovery."""
...
class LocalEpicFileSystem:
    """Use daemon-bounded filesystem calls so local manifest access cannot stall Vigil."""
    ...
    def _bounded(
        ...
        if context.is_cancelled():
            raise TimeoutError(
                "Epic Games discovery was cancelled or its deadline expired"
            )
        ...
        if timeout <= 0.0:
            raise TimeoutError("Epic Games discovery deadline expired")
        ...
        thread = Thread(target=run, name="VigilEpicFileSystem", daemon=True)
        thread.start()
        try:
            succeeded, payload = queue.get(timeout=timeout)
        except Empty as exc:
            raise TimeoutError("Epic Games filesystem operation timed out") from exc
        if not succeeded:
            if isinstance(payload, Exception):
                raise payload
            raise OSError("Epic Games filesystem operation failed")
        return payload  # type: ignore[return-value]


__all__ = ["EpicFileSystem", "LocalEpicFileSystem"]
